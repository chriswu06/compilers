...
(define (compile-until e1 e2)
  (let ((start-loop (gensym 'if))
        (end-loop (gensym 'if))
        (value-not-present (gensym 'if))
        (done (gensym 'if)))
    (seq (Sub rsp 16)
         ...
         (Jne end-loop)
         (compile-e e2)
         (Mov (Offset rsp 0) (value->bits #t))
         (Mov (Offset rsp 8) rax)
         (Jmp start-loop)
         (Label end-loop)
         (Cmp (Offset rsp 0) (value->bits #t))
         (Jne value-not-present)
