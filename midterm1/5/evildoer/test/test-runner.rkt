...
(define (test/io run)
  ...
  (begin ;; Midterm
    (check-equal? (run "" '(begin (write-byte 97) 42)) (cons 42 "a"))
    (check-equal? (run "" '(begin (write-byte 97) (write-byte 98) 42)) (cons 42 "ab"))
    (check-equal? (run "" '(begin0 13 (write-byte 97))) (cons 13 "a"))
    (check-equal? (run "" '(begin0 13 (write-byte 97) (write-byte 98))) (cons 13 "ab"))))
