      ...
      [(list 'or s1 s2)
       (match (rec s1 xs ys gs)
         [(list ys xs1 gs p1)
          (match (rec s2 xs ys gs)
            [(list ys xs2 gs p2)
             (let ((unbound (set-union (set-subtract xs1 xs2)
                                       (set-subtract xs2 xs1))))
               (if (not (empty? unbound))
                   (error "variable not bound in both or patterns" (car unbound))
                   (if (not (equal? xs1 xs2))
                       (error "variables not bound in same order in both or patterns" p)
                       (list ys xs1 gs (Disj p1 p2)))))])
          (match (rec s2 xs ys gs)
            [(list ys xs gs p2)
             (list ys xs gs (Disj p1 p2))])])]
