...
(define (test run)
  ;; NEW
  (begin ;; list-ref tests
    (check-equal? (run '(list-ref '() 0)) 'err)
    (check-equal? (run '(list-ref (cons #\a '()) 2)) 'err)
    (check-equal? (run '(list-ref (cons #\a '()) 0)) #\a)
    (check-equal? (run '(list-ref (cons #\a #\b) 0)) #\a))

  ;; NEW
  (begin ;; memq tests
    (check-equal? (run '(memq 1 '())) #f)
    (check-equal? (run '(memq #\a (cons #\a '()))) (list #\a))
    (check-equal? (run '(memq #\a (cons #\a (cons #\a '())))) (list #\a #\a))
    (check-equal? (run '(memq #\b (cons #\a (cons #\b '())))) (list #\b)))

  ;; NEW
  (begin ;; proper memq tests
    (check-equal? (run '(memq #\a (cons #\a #\b))) 'err))
