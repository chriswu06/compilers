    ...
    ['list-ref
     (let ((beginloop (gensym 'beginloop))
          ...
          (seq (Pop r8)
               ...
               (Label beginloop)
               (Cmp rax 0)
               (Je done) ;;Take car if index is 0
               (assert-cons r8) ;;Bounds check: cons cell must still be a cons
               (Mov r8 (Mem r8 (- 8 type-cons))) ;;r8 = cdr(r8)
               (Sub rax (value->bits 1))
               (Jmp beginloop) ;;Search again w/ index-1

               (Label done)
               (assert-cons r8)
               (Mov rax (Mem r8 (- 0 type-cons)))))]
    ['memq
     (let ((beginloop (gensym 'beginloop))
          ...
          (seq (Pop r8)
               ...
               (Je notfound)
               (assert-cons rax)
               (Mov r9 (Mem rax (- 0 type-cons)))
               (Cmp r9 r8) ;;if car(lst) =? v, return tail
               (Je done)
               (Mov rax (Mem rax (- 8 type-cons))) ;;rax = cdr(lst)
