...
(define (test run)
  ...
  (begin
    ;; NEW
    (begin ;; vector-memq tests
      (check-equal? (run '(let ([v (make-vector 0 #\a)]) (vector-memq #\a v))) #f)
      (check-equal? (run '(let ([v (make-vector 1 #\a)]) (vector-memq #\a v))) 0)
      (check-equal? (run '(let ([v (make-vector 3 #\a)])
                            (begin (vector-set! v 1 #\b)
                                   (begin (vector-set! v 2 #\c)
                                          (vector-memq #\b v)))))
                    1))

    ;; NEW
    (begin ;; vector-copy! tests
      (check-equal? (run '(let ([v1 (make-vector 3 #\a)])
                            (let ([v2 (make-vector 2 #\b)])
                              (begin (vector-copy! v1 0 v2)
                                     v1))))
                    #(#\b #\b #\a))
      (check-equal? (run '(let ([v1 (make-vector 3 #\a)])
                            (let ([v2 (make-vector 2 #\b)])
                              (begin (vector-copy! v1 1 v2)
                                     v1))))
                    #(#\a #\b #\b))
      (check-equal? (run '(let ([v1 (make-vector 3 #\a)])
                            (let ([v2 (make-vector 2 #\b)])
                              (begin (vector-copy! v1 2 v2)
                                     v1))))
                    'err)))
