...
(define (compile-op2 p)
  (match p
    ...
    ['vector-memq
     (let ((beginloop (gensym 'beginloop))
           (found (gensym 'found))
           (done (gensym 'done))
           (notfound (gensym 'notfound)))
          ...
          (seq (Pop r8)
               ...
               (Cmp r10 r9) ;notfound
               (Jge notfound)
               (Mov rax (Mem r8 r10 (- 8 type-vect))) ;rax = vec[r10]
               (Cmp rax r11) ;vec[r10] =? v
               (Je found)
               (Add r10 8)
               (Jmp beginloop) ;search again
    ...
    ['vector-copy!
     (let ((beginloop (gensym 'beginloop))
          ...
          (seq (Pop r8)
               ...
               (assert-vector r9)
               (assert-natural r8)
               (Mov r10 (Mem rax (- type-vect)))
               (Mov r11 (Mem r9 (- type-vect)))

               ;;Bounds-checking & no elements copied
               ;;Check dest-start index is in bounds of dest vector
               (Cmp r8 r11)
               (Jge 'err)
               ;;Check length of src plus the dest-start index is inside bounds of dest vector
               (Mov rcx r8)
               (Add rcx r10)
               (Cmp rcx r11)
               (Jg 'err)
               ...
               (Label beginloop)
               (Cmp rcx r10) ;;Finished copying
               (Jge done)
               (Mov rdx (Mem rax rcx (- 8 type-vect))) ;
               (Mov (Mem r9 r8 (- 8 type-vect)) rdx) ; dest[r8] = src[rcx]
               (Add rcx 8)
               (Add r8 8)
               (Jmp beginloop) ;;Copy more
