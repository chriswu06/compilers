...
(define (interp-prim2 op v1 v2)
  (match (list op v1 v2)
    ;; NEW
    [(list 'vector-memq _ (? vector?)) (vector-memq v1 v2)]
...
(define (interp-prim3 p v1 v2 v3)
  (match (list p v1 v2 v3)
    ;; NEW
    [(list 'vector-copy! (? vector?) (? integer?) (? vector?))
     (let ([dst-len (vector-length v1)]
           [src-len (vector-length v3)])
       (if (and (<= 0 v2 dst-len)
                (<= (+ v2 src-len) dst-len))
           (vector-copy! v1 v2 v3)
           (raise 'err)))]
